...
<?php if (!defined('MIDDLEWARE_INCLUDED')) { require_once __DIR__ . '/config/middleware.php'; } require_permission('whms_pickup', 'is_view'); ?>
                    ...
                    function loadStatuses() {
                        $.get('api/master_status/read.php?length=-1&status=active', function (response) {
                            if (response.data) {
                                ...
                                // Default: select "Picked Up" if present
                                var pickedUp = response.data.find(function (s) {
                                    return s.name && String(s.name).toLowerCase().replace(/\s+/g, ' ') === 'picked up';
                                });
                                if (pickedUp) {
                                    $select.val(pickedUp.name);
                                }
                                ...
                                setTimeout(function () {
                                    ...
                                    if (pickedUp) {
                                        $select.val(pickedUp.name).trigger('change');
                                    }
                                }, 100);
                            }
                        });
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.status === 'success') {
                                        ...
                                        // Refresh tracking history (common status + tracking updated)
                                        loadTrackingHistory(bookingId, itemKey);
