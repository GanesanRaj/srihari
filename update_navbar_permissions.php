...
echo "<h2>Updating Navbar Permission Structure</h2>";
...
$navbar_structure = [
    ...
    // Module 7: Serial Allocation
    [
        'module_name' => 'Serial Allocation',
        'sorted' => 7,
        'permissions' => [
    ...
    // Module 9: Reports & Tools
    [
        'module_name' => 'Reports & Tools',
        'sorted' => 9,
        'permissions' => [
...
try {
    ...
    // Clear existing modules and permissions (optional - comment out if you want to keep existing)
    // $pdo->exec("DELETE FROM staff_privileges");
    // $pdo->exec("DELETE FROM permission");
    // $pdo->exec("DELETE FROM permission_modules");
    
    $module_id_start = 1;
    $permission_id_start = 1;
    
    foreach ($navbar_structure as $module) {
        // Insert or update module
        $module_prefix = strtolower(str_replace(' ', '_', $module['module_name']));
        ...
        echo "<p style='color: green;'>Module: {$module['module_name']} - Added/Updated</p>";
        ...
        foreach ($module['permissions'] as $perm) {
            ...
            echo "<p style='color: blue;'>  - Permission: {$perm['name']} ({$perm['prefix']})</p>";
