            ...
            <div class="px-0">
                ...
                <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
                    <div class="flex-grow-1">
                        <h4 class="fs-18 fw-semibold m-0">Delhivery B2C Booking</h4>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Delhivery B2C Shipment Booking</h4>
                            </div>
                            <div class="card-body">
                                        ...
                                        <div class="tab-content pt-3" data-wizard-content>
                                            ...
                                            <div class="tab-pane fade" id="packageInfo">
                                                ...
                                                <div class="d-flex align-items-center justify-content-between mb-1">
                                                    <h5 class="mb-0">Delhivery Multi-Box Package Details</h5>
                                                    <button type="button" class="btn btn-sm btn-outline-success" id="btnGenerateAllAwbs">
                                                        <i class="ti ti-refresh"></i> Generate All AWBs
                                                    </button>
                                                </div>
                                                <p class="text-muted fs-xxs mb-2">
                                                    Each row is one box. AWB is auto-fetched from Delhivery if empty, or enter manually.
                                                </p>
                                                            ...
                                                            <tr class="pkg-row">
                                                                ...
                                                                <td>
                                                                    <div class="input-group input-group-sm">
                                                                        <input type="text" name="pkg_awb_no[]"
                                                                            class="form-control form-control-sm awb-input"
                                                                            placeholder="Auto or enter AWB">
                                                                        <button type="button" class="btn btn-outline-secondary btn-gen-awb" title="Generate AWB from Delhivery">
                                                                ...
                                                                <td><input type="number" name="boxes[]"
                                                                        class="form-control form-control-sm calc-trigger bg-light"
                                                                        min="1" step="1" value="1" required readonly></td>
...
<script>
    $(document).ready(function () {
        ...
        // Load Couriers — auto-select Delhivery B2C on page load
        $.get('api/courier_partner/read.php?length=-1', function (res) {
            if (res.data) {
                ...
                res.data.forEach(c => {
                    ...
                    // Auto-select the first courier whose name contains 'delhivery' (case-insensitive)
                    if (!delhiveryId && c.partner_name.toLowerCase().indexOf('delhivery') !== -1) {
                        delhiveryId = c.id;
                    }
                });
                if (delhiveryId) {
                    $('#courier_id').val(delhiveryId).trigger('change');
                    if (typeof $ !== 'undefined' && $.fn.select2) {
                        $('#courier_id').trigger('change.select2');
                    }
                }
            }
        });
        ...
        // Payment Mode — show COD Amount row only when COD is selected
        function toggleCodAmount() {
            var mode = $('select[name="payment_mode"]').val();
            if (mode === 'COD') {
                $('#codAmountRow').show();
                $('#cod_amount_input').prop('required', true);
            } else {
                $('#codAmountRow').hide();
                $('#cod_amount_input').prop('required', false).val(0);
            }
        }
        $('select[name="payment_mode"]').change(toggleCodAmount);
        toggleCodAmount(); // initialize on load
        ...
        <tr class="pkg-row">
            ...
            <td>
                <div class="input-group input-group-sm">
                    <input type="text" name="pkg_awb_no[]" class="form-control form-control-sm awb-input" placeholder="Auto or enter AWB" value="${rowData.pkg_awb_no || ''}">
                    <button type="button" class="btn btn-outline-secondary btn-gen-awb" title="Generate AWB from Delhivery"><i class="ti ti-refresh"></i></button>
                </div>
            </td>
            ...
            <td><input type="number" name="boxes[]" class="form-control form-control-sm calc-trigger bg-light" min="1" step="1" value="${rowData.boxes || 1}" required readonly></td>
        ...
        // ── Delhivery AWB Auto-Generation ──────────────────────────────────
        // Fetches a waybill from Delhivery for a single row (if AWB field is empty)
        function fetchAwbForRow($row, callback) {
            var $input  = $row.find('.awb-input');
            ...
            if (!courierId) {
                if (typeof showtoastt === 'function') showtoastt('Select a courier first', 'warning');
                return;
            }
            var $btn = $row.find('.btn-gen-awb');
            $btn.prop('disabled', true).html('<i class="ti ti-loader-2 ti-spin"></i>');
            $.get('api/delhivery/fetch_waybill.php', { courier_id: courierId, count: 1 }, function (res) {
                $btn.prop('disabled', false).html('<i class="ti ti-refresh"></i>');
                if (res.status === 'success' && res.waybills && res.waybills.length) {
                    $input.val(res.waybills[0]).addClass('is-valid').removeClass('is-invalid');
                    if (typeof callback === 'function') callback(res.waybills[0]);
                } else {
                    $input.addClass('is-invalid');
                    if (typeof showtoastt === 'function') showtoastt('AWB fetch failed: ' + (res.message || 'Unknown error'), 'error');
                }
            }, 'json').fail(function () {
                $btn.prop('disabled', false).html('<i class="ti ti-refresh"></i>');
                if (typeof showtoastt === 'function') showtoastt('AWB fetch request failed', 'error');
            });
        }

        // Generate AWBs for ALL empty rows in the table at once (batched API call)
        function generateAllAwbs() {
            var courierId = $('#courier_id').val();
            if (!courierId) {
                if (typeof showtoastt === 'function') showtoastt('Select a courier first', 'warning');
                return;
            ...
            var $emptyRows = [];
            $('#pkgTable tbody .pkg-row').each(function () {
                var $input = $(this).find('.awb-input');
                if ($.trim($input.val()) === '') {
                    $emptyRows.push($(this));
                }
            });
            if ($emptyRows.length === 0) {
                if (typeof showtoastt === 'function') showtoastt('All rows already have AWB numbers', 'info');
                return;
            }
            $('#btnGenerateAllAwbs').prop('disabled', true).html('<i class="ti ti-loader-2 ti-spin"></i> Generating...');
            $.get('api/delhivery/fetch_waybill.php', { courier_id: courierId, count: $emptyRows.length }, function (res) {
                $('#btnGenerateAllAwbs').prop('disabled', false).html('<i class="ti ti-refresh"></i> Generate All AWBs');
                if (res.status === 'success' && res.waybills && res.waybills.length) {
                    $emptyRows.forEach(function ($row, idx) {
                        var wb = res.waybills[idx];
                        if (wb) {
                            $row.find('.awb-input').val(wb).addClass('is-valid').removeClass('is-invalid');
                        }
                    });
                    if (typeof showtoastt === 'function') showtoastt('Generated ' + res.waybills.length + ' AWB(s)', 'success');
                } else {
                    if (typeof showtoastt === 'function') showtoastt('AWB fetch failed: ' + (res.message || 'Unknown error'), 'error');
                }
            }, 'json').fail(function () {
        ...
        // Per-row Generate button click
        $(document).on('click', '.btn-gen-awb', function () {
            var $row = $(this).closest('tr.pkg-row');
            fetchAwbForRow($row);
        });

        // Generate All AWBs button
        $('#btnGenerateAllAwbs').click(function () {
            generateAllAwbs();
        });

        // Auto-generate AWBs when Package tab becomes active
        $('[data-wizard-nav] .nav-link[href="#packageInfo"]').on('shown.bs.tab click', function () {
            setTimeout(function () { generateAllAwbs(); }, 200);
        });
        // Also handle Bootstrap tab switch event on the tab pane itself
        $('#packageInfo').on('show.bs.tab', function () {
            setTimeout(function () { generateAllAwbs(); }, 200);
        });
        ...
        const tabValidationRules = {
            ...
            confirmInfo: [
                ...
                { name: "invoice_no", message: "Invoice No is required", required: true },
                { name: "invoice_value", message: "Invoice Value is required", required: true }
            ]
        };
