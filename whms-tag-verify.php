...
$is_super_admin = (isset($_SESSION['user_id']) && (int)$_SESSION['user_id'] === 1) || (isset($_SESSION['role_id']) && (int)$_SESSION['role_id'] === 1);
?>
        ...
        <div class="content-page">
            <div class="content">
                <div class="px-0">
                    ...
                    <!-- Step 1: CD No / Manifest No (optional – shows overall and tag/shipment counts) -->
                    <div class="row mb-1">
                        <div class="col-12">
                            <div class="card mb-0">
                                <div class="card-body py-2 d-flex align-items-center gap-3 flex-wrap">
                                    <div class="input-group input-group-sm" style="width: 320px;">
                                        <span class="input-group-text fw-bold fs-12">CD No / Manifest No</span>
                                        <input type="text" id="cdOrManifestInput" class="form-control" placeholder="Enter CD No or Manifest No..." autocomplete="off">
                                        <button class="btn btn-outline-primary btn-sm" id="btnLoadCdManifest" type="button"><i class="ti ti-search"></i> Load</button>
                                    </div>
                                    <div id="manifestSummary" class="d-none d-flex gap-3 align-items-center flex-wrap">
                                        <span class="badge bg-primary px-3 py-2">Overall: <b id="summaryOverall">0</b></span>
                                        <span class="badge bg-info px-3 py-2" id="tagsBadge" title="">Tags: <b id="summaryTags">0</b></span>
                                        <span class="badge bg-secondary px-3 py-2">Shipments: <b id="summaryShipments">0</b></span>
                                        <span class="text-muted fs-12" id="summaryManifestNo"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Step 2: Controls Bar (Tag search + verify) -->
                    <div class="row mb-1">
                        <div class="col-12">
                            <div class="card mb-0">
                                <div class="card-body py-2 d-flex align-items-center gap-3 flex-wrap">
            ...
            <script>
                ...
                function displayAwb(awb) {
                    if (!awb) return '';
                    if (isSuperAdmin) return awb;
                    var len = awb.length;
                    if (len <= 4) return awb.slice(0, 2) + '***' + (len >= 2 ? awb.slice(-2) : '');
                ...
                $(function () {
                    ...
                    /** When manifest loaded by CD/Manifest no: only these tag numbers are allowed for scan */
                    let manifestTagNos = [];
                    /** True after user has loaded a manifest by CD No / Manifest No (required before scanning any tag) */
                    let manifestLoadedByCdOrManifest = false;
                    ...
                    // ── Step 1: Load by CD No or Manifest No ───────────────────────
                    $('#cdOrManifestInput').on('keydown', function (e) { if (e.key === 'Enter') { e.preventDefault(); loadCdOrManifest(); } });
                    $('#btnLoadCdManifest').on('click', loadCdOrManifest);

                    function loadCdOrManifest() {
                        const val = $('#cdOrManifestInput').val().trim();
                        if (!val) return;
                        // Try manifest_no first, then cd_no
                        $.get('api/manifest/readone.php?manifest_no=' + encodeURIComponent(val), res => {
                            if (res.status === 'success' && res.data) {
                                showManifestSummary(res.data);
                                return;
                            }
                            $.get('api/manifest/readone.php?cd_no=' + encodeURIComponent(val), res2 => {
                                if (res2.status === 'success' && res2.data) {
                                    showManifestSummary(res2.data);
                                } else {
                                    $('#manifestSummary').addClass('d-none');
                                    manifestTagNos = [];
                                    manifestLoadedByCdOrManifest = false;
                                    alert('No manifest found for: ' + val);
                                }
                            });
                        });
                    }

                    function showManifestSummary(data) {
                        const shipmentCount = data.shipment_count != null ? data.shipment_count : (Array.isArray(data.json_data) ? data.json_data.length : 0);
                        const tagsCount = data.tags_count != null ? data.tags_count : 0;
                        $('#summaryOverall').text('1');
                        $('#summaryTags').text(tagsCount);
                        $('#summaryShipments').text(shipmentCount);
                        $('#summaryManifestNo').text((data.manifest_no || '') + (data.cd_no ? ' | CD: ' + data.cd_no : ''));
                        $('#manifestSummary').removeClass('d-none');
                        manifestLoadedByCdOrManifest = true;
                        // Restrict tag scan to only tags that appear in this manifest's json_data
                        const entries = Array.isArray(data.json_data) ? data.json_data : [];
                        manifestTagNos = [...new Set(entries.map(function (e) { return (e.tag_no || '').trim(); }).filter(Boolean))];
                        // Tooltip: inside tags + verified count
                        var tip = [];
                        if (manifestTagNos.length) tip.push('Tags: ' + manifestTagNos.join(', '));
                        var tagVerified = data.tag_verified_counts || [];
                        var totalVerified = (data.total_verified_count != null) ? data.total_verified_count : 0;
                        if (tagVerified.length) {
                            tagVerified.forEach(function (t) { tip.push(t.tag_no + ': ' + (t.verified_count || 0) + ' verified'); });
                        }
                        tip.push('Total verified: ' + totalVerified);
                        $('#tagsBadge').attr('title', tip.join('\n'));
                    }
                    ...
                    function loadTag() {
                        ...
                        if (!tagNo) return;
                        // Require manifest to be loaded first (by CD No or Manifest No)
                        if (!manifestLoadedByCdOrManifest) {
                            beepError();
                            alert('Please load by CD No or Manifest No first. Only tags belonging to that manifest can be scanned.');
                            $('#tagSearch').focus();
                            return;
                        }
                        // Only allow tags that belong to the loaded manifest
                        if (manifestTagNos.length > 0) {
                            const allowed = manifestTagNos.some(function (t) { return t === tagNo; });
                            if (!allowed) {
                                beepError();
                                alert('This tag is not in the loaded manifest (e.g. ' + (manifestTagNos[0] || '') + '). Scan only tags that are in this manifest.');
                                $('#tagSearch').val('').focus();
                                return;
                            }
                        }
                    ...
                    function appendShipRow(entry, idx) {
                        ...
                        const tr = `<tr class="${rowCls}" data-awb="${entry.awb_no}">
                            ...
                            <td><strong style="font-size:12px;">${displayAwb(entry.awb_no)}</strong></td>
