...
<?php
// Superadmin check (role_id = 1 means full admin in this system)
$isSuperAdmin = ((int) ($_SESSION[ 'role_id' ] ?? 0) === 1);

// Optional contextual labels (can be overridden by wrapper pages)
$bulkPageTitle = isset($bulkPageTitle) && trim((string)$bulkPageTitle) !== ''
    ? (string)$bulkPageTitle
    : 'Bulk Shipment Upload';
$bulkCardTitle = isset($bulkCardTitle) && trim((string)$bulkCardTitle) !== ''
    ? (string)$bulkCardTitle
    : 'Upload Excel/CSV';
$bulkHistoryTitle = isset($bulkHistoryTitle) && trim((string)$bulkHistoryTitle) !== ''
    ? (string)$bulkHistoryTitle
    : 'Upload History';
?>
<!-- Vendors CSS -->
            ...
            <div class="px-0">
                ...
                <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
                    <div class="flex-grow-1">
                        <h4 class="fs-18 fw-semibold m-0"><?php echo htmlspecialchars($bulkPageTitle, ENT_QUOTES, 'UTF-8'); ?></h4>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h4 class="card-title"><?php echo htmlspecialchars($bulkCardTitle, ENT_QUOTES, 'UTF-8'); ?></h4>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"><?php echo htmlspecialchars($bulkHistoryTitle, ENT_QUOTES, 'UTF-8'); ?></h4>
                            </div>
                            <div class="card-body">
    ...
    $(document).ready(function () {
        ...
        columns = columns.concat([
            { data: 'id' },
            {
                data: 'created_at',
                render: function (v) {
                    return v ? v.replace(/^(\d{4}-\d{2}-\d{2})\s.*/, '$1') : '-';
                }
            },
            { data: 'filename' },
            {
            ...
            },
            {
                data: 'failure_count',
                render: function (d) {
                    return `<span class="text-danger fw-bold">${d}</span>`;
                }
            },
            {
                data: 'status',
            ...
            },
            {
                ...
                render: function (id, type, row) {
                    ...
                    if (row.result_file) {
                        html += `<a href="api/shipment/export_result.php?id=${id}" class="btn btn-sm btn-info" download>Download Result</a>`;
                    }
                    return html || '-';
                }
            }
        ...
        if (isSuperAdmin) {
            // Action column with single delete
            columns.push({
                data: 'id',
                orderable: false,
                searchable: false,
