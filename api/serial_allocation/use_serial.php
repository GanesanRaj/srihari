...
$remarks = isset($_POST['remarks']) ? sanitizeText($_POST['remarks']) : 'Used for booking';
...
try {
    ...
    $checkSql = "SELECT * FROM tbl_serial_numbers WHERE serial_number = :serial_number AND is_used = 0 AND status IN ('available', 'reserved', 'cancelled')";
    ...
    if (!$serial) {
        $pdo->rollBack();
        echo json_encode(['status' => 'error', 'message' => 'Serial number not available or already used']);
        exit;
    }

    // Update serial number as used
    ...
    $historySql = "INSERT INTO tbl_serial_history (serial_number_id, serial_number, branch_id, action, booking_id, performed_by, action_date, remarks)
                   VALUES (:serial_number_id, :serial_number, :branch_id, 'used', :booking_id, :performed_by, NOW(), :remarks)";
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'Serial number used successfully',
