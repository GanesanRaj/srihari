...
/**
 * Consignee List API
 * Location: /apps-api/consignee/list.php
 * Filters: branch_id, client_id, status, search. No pickup_point (app list is not pickup-point-based).
 */
...
try {
    $sql = "SELECT c.*, b.branch_name, cl.client_name, co.name as consignor_name
            FROM tbl_consignee c
            LEFT JOIN tbl_branch b ON c.branch_id = b.id
            LEFT JOIN tbl_client cl ON c.client_id = cl.id
            LEFT JOIN tbl_consignor co ON c.client_id = co.id
            WHERE 1=1";
    
    $countSql = "SELECT COUNT(*) as total FROM tbl_consignee c WHERE 1=1";
    $params = [];
