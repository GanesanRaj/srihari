...
$status      = trim($req['status'] ?? 'Delivered');
...
$image_folder = trim($req['images_folder'] ?? 'delivery/' . date('Y/m/d'));
...
try {
    // Resolve awb_no to booking: child or parent
    ...
    // Delivery POD images (compress 200KB)
    $deliveryPodImages = [];
    if (isset($_FILES['delivery_pod_images']) && is_array($_FILES['delivery_pod_images']['name'])) {
        $maxKb = 200;
        for ($i = 0, $n = count($_FILES['delivery_pod_images']['name']); $i < $n; $i++) {
            if ($_FILES['delivery_pod_images']['error'][$i] !== UPLOAD_ERR_OK) continue;
            $fileInput = [
                'name' => $_FILES['delivery_pod_images']['name'][$i],
                'tmp_name' => $_FILES['delivery_pod_images']['tmp_name'][$i],
                'error' => UPLOAD_ERR_OK,
                'size' => $_FILES['delivery_pod_images']['size'][$i],
            ];
            $path = handle_image_upload($fileInput, $image_folder, null, $maxKb);
            if ($path) $deliveryPodImages[] = $path;
        }
    }
    ...
    try {
        ...
        if ($isChild && $packageId > 0) {
            ...
            $updPkg = $pdo->prepare("UPDATE tbl_booking_packages SET status = :status, delivery_date = :dt, delivery_pod_images = :imgs, remarks = :rem, updated_by = :uid, updated_at = NOW() WHERE id = :id");
            $updPkg->execute([
                ':status' => $status, ':dt' => $statusDateTime, ':imgs' => json_encode($deliveryPodImages),
            ...
            if (count($unique) === 1 && strtolower(reset($unique)) === 'delivered') {
                ...
                $pdo->prepare("UPDATE tbl_bookings SET last_status = 'Partially Delivered', updated_by = :uid, updated_at = NOW() WHERE id = :id")
                    ->execute([':uid' => $user_id, ':id' => $bookingId]);
            }
        } else {
            // Update all packages for the booking
            $pdo->prepare("UPDATE tbl_bookings SET last_status = :status, updated_by = :uid, updated_at = NOW() WHERE id = :id")
                ->execute([':status' => $status, ':uid' => $user_id, ':id' => $bookingId]);
            
            $pdo->prepare("UPDATE tbl_booking_packages SET status = :status, delivery_date = :dt, delivery_pod_images = :imgs, remarks = :rem, updated_by = :uid, updated_at = NOW() WHERE booking_id = :bid")
                ->execute([
                    ':status' => $status, ':dt' => $statusDateTime, ':imgs' => json_encode($deliveryPodImages),
        ...
        $trackingId = null;
        if ($shouldWriteTracking || strtolower($status) === 'delivered') {
            ...
            $newScan = [
                ...
                'remarks' => $remarks,
                'delivery_pod_images' => $deliveryPodImages,
                'delivery_pod_image_count' => count($deliveryPodImages),
                'updated_by' => $user_id,
            ...
            if (function_exists('syncShipmentStatusAcrossTables')) {
                syncShipmentStatusAcrossTables($pdo, $bookingId, $waybillNo, $finalStatus, $remarks, 'app-delivery-' . $user_id);
            }
        }

        $pdo->commit();

        $bookingStatus = $shouldWriteTracking ? $finalStatus : 'Partially Delivered';
        echo json_encode([
            'status' => 'success',
            'message' => $shouldWriteTracking
                ? 'Delivery status updated successfully' . (count($deliveryPodImages) > 0 ? ' with ' . count($deliveryPodImages) . ' image(s)' : '')
                : 'Package updated. Booking is Partially Delivered.',
            ...
            'delivery_pod_images' => $deliveryPodImages,
            'image_count' => count($deliveryPodImages),
        ]);
    } catch (Exception $e) {
        $pdo->rollBack();
        throw $e;
