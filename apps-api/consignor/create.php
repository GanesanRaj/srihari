<?php
/**
 * Consignor Create API
 * Location: /apps-api/consignor/create.php
 */
...
if (empty($req['branch_id']) || empty($req['client_id']) || empty($req['name']) || empty($req['contact_no']) || empty($req['address']) || empty($req['city']) || empty($req['state']) || empty($req['pincode'])) {
    echo json_encode(['status' => 'error', 'message' => 'Required fields are missing']);
    exit();
}

try {
    $sql = "INSERT INTO tbl_consignor (branch_id, client_id, name, contact_no, alt_contact_no, email, gst_number, address, location, city, state, pincode, status, created_by) 
    ...
    if ($stmt->execute()) {
        echo json_encode(['status' => 'success', 'message' => 'Consignor created successfully', 'id' => $pdo->lastInsertId()]);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Failed to create consignor']);
    }
